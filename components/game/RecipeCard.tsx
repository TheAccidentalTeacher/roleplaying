...
function getRarityBorder(rarity: ItemRarity): string {
  switch (rarity) {
    case 'junk': return 'border-slate-700';
  ...
  return (
    <button
      ...
      className={`
        ...
        ${unlocked ? 'bg-slate-900 hover:bg-slate-800 cursor-pointer' : 'bg-slate-950 opacity-50 cursor-not-allowed'}
    ...
    >
      <div className="flex items-start justify-between mb-1">
        ...
        <span className="text-xs text-slate-600">DC {craftingDC}</span>
      </div>
